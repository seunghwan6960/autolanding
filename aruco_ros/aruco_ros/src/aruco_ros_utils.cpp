...
tf::Transform aruco_ros::arucoMarker2Tf_for_Tracker(const aruco::MarkerPoseTracker &Tracker, bool check_tracker)
{
  if(check_tracker){ 
  tf2::Transform tf2_tf = arucoMarker2Tf2_for_Tracker(Tracker);
  ...
  return tf::Transform(
    ...
    ROS_ERROR("invalid Tracker!!!!");
  }
}
tf2::Transform aruco_ros::arucoMarker2Tf2_for_Tracker(const aruco::MarkerPoseTracker &Tracker)
{

  cv::Mat rot(3, 3, CV_64FC1);
  cv::Mat Rvec64;
  cv::Mat R_tmp;cv::Mat T_tmp;
  R_tmp=Tracker.getRvec();
  
  R_tmp.convertTo(Rvec64, CV_64FC1);
  std::cout<<R_tmp.size()<<std::endl;
  ROS_ERROR("******************");
  ...
  T_tmp=Tracker.getTvec();
  
  T_tmp.convertTo(tran64, CV_64FC1);
